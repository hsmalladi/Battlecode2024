...
import static jobsnotfinished.BotSetupExploreDuck.checkValidTrap;
...
public class BotMainRoundAttackDuck extends BotMainRoundDuck {
    ...
    public static void tryTrap() throws GameActionException {
        if (rc.getLevel(SkillType.BUILD) >= 3) {
            ...
            if (oppRobotInfos.length >= 5) {
                ...
                if (build && rc.canBuild(TrapType.STUN, me.add(dir)) && checkValidTrap(me.add(dir), dir)) {
                    rc.build(TrapType.STUN, me.add(dir));
                }

                build = true;
                for (MapLocation adj : Map.getAdjacentLocations(me.add(dir.rotateLeft()))) {
                    if (rc.canSenseLocation(adj)) {
                        if (rc.senseMapInfo(adj).getTrapType() == TrapType.STUN){
                            build = false;
                        }
                ...
                if (build && rc.canBuild(TrapType.STUN, me.add(dir.rotateLeft())) && checkValidTrap(me.add(dir.rotateLeft()), dir)) {
                    rc.build(TrapType.STUN, me.add(dir.rotateLeft()));
                }

                build = true;
                for (MapLocation adj : Map.getAdjacentLocations(me.add(dir.rotateRight()))) {
                    if (rc.canSenseLocation(adj)) {
                        if (rc.senseMapInfo(adj).getTrapType() == TrapType.STUN){
                            build = false;
                        }
                ...
                if (build && rc.canBuild(TrapType.STUN, me.add(dir.rotateRight())) && checkValidTrap(me.add(dir.rotateRight()), dir)) {
                ...
                if (build && rc.canBuild(TrapType.STUN, me) && checkValidTrap(me, dir)) {
            ...
            if (rc.getCrumbs() > 300) {
                if (oppRobotInfos.length > 0) {
                    ...
                    if (build) {
                        if (rc.canBuild(TrapType.STUN, me.add(dir)) && checkValidTrap(me.add(dir), dir) && rc.getActionCooldownTurns() < 5) {
                            rc.build(TrapType.STUN, me.add(dir));
                        }
                    }
                    ...
                    if (build) {
                        if (rc.canBuild(TrapType.STUN, me) && checkValidTrap(me, dir) && rc.getActionCooldownTurns() < 5) {
                            rc.build(TrapType.STUN, me);
                        }
                    }
                }
    ...
    public static void tryWaterTrap() throws GameActionException {
        ...
        for (RobotInfo opps : oppRobotInfos) {
            if (opps.hasFlag()) {
                ...
                else if (rc.canBuild(TrapType.WATER, me.add(dir.rotateLeft())) && rc.getActionCooldownTurns() < 5) {
                    rc.build(TrapType.WATER, me.add(dir.rotateLeft()));
                }
                else if (rc.canBuild(TrapType.WATER, me.add(dir.rotateRight())) && rc.getActionCooldownTurns() < 5) {
                    rc.build(TrapType.WATER, me.add(dir.rotateRight()));
                }
                else if (rc.canBuild(TrapType.WATER, me) && rc.getActionCooldownTurns() < 5) {
    ...
    private static void tryHeal() throws GameActionException {
        ...
        if (enemyRobots.length > 0){
            /*int healLvl = rc.getLevel(SkillType.HEAL);
            int attackLvl = rc.getLevel(SkillType.ATTACK);
            int buildLvl = rc.getLevel(SkillType.BUILD);
            if (buildLvl > 3) {
