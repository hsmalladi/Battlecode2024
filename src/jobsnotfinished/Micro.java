...
public class Micro extends Globals {
    ...
    boolean doMicro(){try{
        ...
        int health = Constants.BASE_CRITICAL_HEALTH;
        health += 10 * (rc.getLevel(SkillType.BUILD) + rc.getLevel(SkillType.ATTACK) + rc.getLevel(SkillType.HEAL));
        severelyHurt = hurt(rc.getHealth(), health);
        ...
        for (RobotInfo unit : units) {
            ...
            if (stunned) {
                System.out.println(unit.getLocation() + " STunned");
                continue;
            }
    ...
    static boolean hurt(int h, int health){
        return h < health;
    }
